...
```O(1):
Key observation: A full bottle of water = A empty bottle + One bottle of water.
...
(numExchange - 1) empty bottles can exchange one bottle of water, unless you have only numExchange left.

we need to get the floor value of numBottles / (numExchange - 1),
that is why we need to minus 1 additionally in the numerator: (numBottles - 1)/ (numExchange - 1).
