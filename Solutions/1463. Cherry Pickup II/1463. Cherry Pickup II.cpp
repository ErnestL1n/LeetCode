...
public:
    bool ok(int val,int mx){
        return 0<=val and val<mx;
    }
    int cherryPickup(vector<vector<int>>& grid) {
        vector<int> dirs={-1,0,1};
        int m=grid.size(),n=grid[0].size();
        VEC<int,3> dp(m,n,n,-1);
        int c1=0,c2=n-1;
        dp[0][c1][c2]=grid[0][c1]+grid[0][c2];
        int res=dp[0][c1][c2];
        rep(i,1,m){
            rep(j,0,n){
                rep(k,0,n){
                    ...
                    if(prev>=0){
                        repall(d1,dirs){
                            ...
                            repall(d2,dirs){
                                c2=d2+k;
                                if(ok(c1,n) and ok(c2,n)){
                                    dp[i][c1][c2]=max(dp[i][c1][c2],prev+(c1==c2?grid[i][c1]:(grid[i][c1]+grid[i][c2])));   
                                    res=max(res,dp[i][c1][c2]); 
                                }
                            }
                        }
                    }
